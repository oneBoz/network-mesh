...
2. **This device up.** `docker compose up -d --build`, then `node scripts/smoke.mjs`
   must print `PASS`. If the VM is up, the last lines also show its nodes in
   the converged set.
...
5. **Sanity.** The remote device's card in Command is green (`5/5 alive`).
   Send one `swarm` from the VM's GCS tab: it must appear in this device's
   feed within two seconds with `5/5 agree`. Neutralise it from whichever
   device is responsible so the map is clean.

If step 5 fails, see *If something goes wrong* below before continuing.
...
in about a second, `suspect → dead` about five seconds later, on every node
independently.
